<#
.Synopsis
    Process newly downloaded Magazines
.DESCRIPTION
    Author - Nick Walker
    Date   - 29 September 2019

    This script will move Magazines into the right folders depending on the filename

.EXAMPLE
    Just run the powershell
    .\processMagazines.ps1  
...
function moveMagazineFiles {
    ...
    foreach ($_magFilename in Get-ChildItem -Path $_sourceFolder -Filter "*.pdf")
    {

        ## // show the file to be processed
        writeToScreen "processing file" "$($_magFilename.Name)"
...
moveMagazineFiles "\\bagpuss\public\Magazines\_InProcess"
