...
:: --------------------------------------------------------------------------------
  ...
  :: // Set the locations
  set "_torrentDrive=T:"
  if not exist "%_torrentDrive%"   set "_torrentDrive=\\emily\Torrent"
  ...
  set "_url=timbersaw.myseedbox.site"
  set "_ftpString=walkme70_4111:zutaboha40@timbersaw.myseedbox.site/"
 ...
 :Upload
  ...
  :RunCommand 
    ...
    if /i "%_processFilebot%" EQU "Yes" (
      if @(( Get-Content %ExcludeTorrentFolder% | Where-Object { $_.Contains ("%_folder%") } ).Count) = 0 (
